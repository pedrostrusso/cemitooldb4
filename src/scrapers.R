...
scrape_study_info <- function(gse_id){
    gds_search <- rentrez::entrez_search(db="gds", term=paste0(gse_id, "[ACCN] AND gse[ETYP])"))
...
get_keywords <- function(pubmedid){
    ...
    nodes <- XML::getNodeSet(xml_rec, '//DescriptorName')
    keywords <- unlist(lapply(nodes, XML::xmlValue))
    return(keywords)
}
